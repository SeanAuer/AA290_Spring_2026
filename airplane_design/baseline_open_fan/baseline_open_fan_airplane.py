...
def vehicle_setup(fan_pressure_ratio=1.1, bypass_ratio=36, include_dorsey_penalties=True): 
    ...
    root_airfoil.coordinate_file          = local_path + 'B737a.txt'
    ...
    yehudi_airfoil.coordinate_file        = local_path + 'B737b.txt'
    segment                               = RCAIDE.Library.Components.Wings.Segments.Segment()
    segment.tag                           = 'Yehudi'
    ...
    mid_airfoil.coordinate_file           = local_path + 'B737c.txt'
    segment                               = RCAIDE.Library.Components.Wings.Segments.Segment()
    ...
    tip_airfoil.coordinate_file           = local_path + 'B737d.txt'
    segment                               = RCAIDE.Library.Components.Wings.Segments.Segment()
    segment.tag                           = 'Tip'
    ...
    #------------------------------------------------------------------------------------------------------------------------------------  
    # Propulsor: Starboard Open Rotor (Modeled as Turbofan with Open Rotor Parameters)
    #------------------------------------------------------------------------------------------------------------------------------------         
    open_rotor                                  = RCAIDE.Library.Components.Powertrain.Propulsors.Turbofan() 
    open_rotor.tag                              = 'starboard_propulsor'
    open_rotor.active_fuel_tanks                = ['fuel_tank']   
    open_rotor.origin                           = [[13.72, 4.86,-1.1]] 
    open_rotor.engine_length                    = 2.71
    open_rotor.bypass_ratio                     = bypass_ratio  # Very high bypass ratio for open rotor (default 36)
    open_rotor.design_altitude                  = 35000.0*Units.ft
    open_rotor.design_mach_number               = 0.78   
    open_rotor.design_thrust                    = 35000.0* Units.N
             
    # fan (open rotor characteristics: low pressure ratio, lower efficiency)               
    fan                                         = RCAIDE.Library.Components.Powertrain.Converters.Fan()   
    fan.tag                                     = 'fan'
    fan.polytropic_efficiency                   = 0.80  # Lower efficiency for open rotor vs ducted fan
    fan.pressure_ratio                          = fan_pressure_ratio  # Very low pressure ratio for open rotor (default 1.1)
    
    # Apply Dorsey integration penalties if enabled
    if include_dorsey_penalties:
        # Reduce fan efficiency by 10% for integration losses
        fan.polytropic_efficiency *= 0.90
        print(f"Applied Dorsey integration penalty: Fan efficiency reduced to {fan.polytropic_efficiency:.3f}")
    ...
    open_rotor.low_pressure_compressor            = low_pressure_compressor
    ...
    open_rotor.high_pressure_compressor            = high_pressure_compressor

    # combustor (identical to turbofan, TIT = 1500K)  
    combustor                                   = RCAIDE.Library.Components.Powertrain.Converters.Combustor()   
    combustor.tag                               = 'Comb'
    combustor.efficiency                        = 0.99 
    combustor.alphac                            = 1.0     
    combustor.turbine_inlet_temperature         = 1500  # Keep at 1500K as specified
    ...
    open_rotor.low_pressure_turbine                = low_pressure_turbine
    ...
    open_rotor.high_pressure_turbine               = high_pressure_turbine
    ...
    # fan nozzle (open rotor - no ducting, lower efficiency)             
    fan_nozzle                                  = RCAIDE.Library.Components.Powertrain.Converters.Expansion_Nozzle()   
    fan_nozzle.tag                              = 'fan nozzle'
    fan_nozzle.polytropic_efficiency            = 0.85  # Lower efficiency for open rotor
    fan_nozzle.pressure_ratio                   = 0.99 
    open_rotor.fan_nozzle                       = fan_nozzle
    
    # design open rotor as turbofan
    design_turbofan(open_rotor)
    
    # Nacelle (smaller core nacelle for open rotor)
    nacelle                                     = RCAIDE.Library.Components.Nacelles.Stack_Nacelle()
    nacelle.tag                                 = 'nacelle_1'
    nacelle.length                              = 2.71
    nacelle.diameter                            = 1.5  # Smaller diameter for core-only nacelle
    
    # Apply Dorsey penalties to nacelle drag
    if include_dorsey_penalties:
        # Increase nacelle wetted area by 40% for propeller slipstream effects (more aggressive)
        base_wetted_area = 1.0
        nacelle.areas.wetted = base_wetted_area * 1.40  # 40% penalty
        print(f"Applied Dorsey penalty: Nacelle wetted area increased by 40%")
    else:
    ...
    # Add nacelle segments (simplified)
    ...
    open_rotor.nacelle                          = nacelle

    # append propulsor to network    
    net.propulsors.append(open_rotor)  

    #------------------------------------------------------------------------------------------------------------------------------------  
    # Propulsor: Port Open Rotor Propulsor
    #------------------------------------------------------------------------------------------------------------------------------------      
    # copy open rotor
    open_rotor_2                                = deepcopy(open_rotor)
    open_rotor_2.active_fuel_tanks              = ['fuel_tank'] 
    open_rotor_2.tag                            = 'port_propulsor' 
    open_rotor_2.origin                         = [[13.72,-4.38,-1.1]]  # change origin 
    open_rotor_2.nacelle.tag                    = 'port_propulsor_nacelle'
    open_rotor_2.nacelle.origin                 = [[13.5,-4.38,-1.5]]
         
    # append propulsor to network
    net.propulsors.append(open_rotor_2)
    ...
    fuel_line.assigned_propulsors =  [[open_rotor.tag, open_rotor_2.tag]]

    #------------------------------------------------------------------------------------------------------------------------------------   
    # Append fuel line to fuel line to network      
    net.fuel_lines.append(fuel_line)        
    
    # Apply Dorsey weight penalties for open rotor integration
    if include_dorsey_penalties:
        # Add noise shielding weight penalty (more aggressive)
        noise_shielding_weight = 0.04 * vehicle.mass_properties.max_takeoff  # 4% MTOW penalty
        
        # Add structural reinforcement weight for open rotor loads
        structural_penalty = 0.03 * vehicle.mass_properties.max_takeoff  # 3% MTOW penalty
        
        # Dorsey approach: Keep MTOW constant, penalties reduce available payload/fuel
        vehicle.mass_properties.operating_empty += noise_shielding_weight + structural_penalty
        # DO NOT increase max_takeoff - let penalties eat into payload/fuel capacity
        
        print(f"Applied Dorsey weight penalties:")
        print(f"  - Noise shielding: {noise_shielding_weight:.0f} kg")
        print(f"  - Structural reinforcement: {structural_penalty:.0f} kg")
        print(f"  - Total penalty: {noise_shielding_weight + structural_penalty:.0f} kg")
        print(f"  - MTOW kept constant, penalties reduce available payload/fuel")
